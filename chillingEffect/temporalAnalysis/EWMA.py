...
def outputResult(peakList, busidicpath, outputpath):
    outputfile = open(outputpath, 'w')
    busiDic = readBusiDic(busidicpath)
    for i in range(0, len(peakList)):
        if len(peakList[i]) == 0:
            continue
        weekId = peakList[i][0][0]
...
if __name__ == '__main__':
    ...
    for i in range(0, numB):
        ts = readTS(basepath + 'individuals/' + str(i))
        topkList = predict(ts, window, topk,basepath,i)
#         print topkList
        peakList.append(topkList)
    outputResult(peakList, basepath + 'filenameDic', basepath + 'spikes')
